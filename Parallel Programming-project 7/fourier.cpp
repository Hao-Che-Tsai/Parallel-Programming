...
main( int argc, char *argv[ ] )
{
	...
	if( me == BOSS )
	{
		...
		for( int dst = 0; dst < NumCpus; dst++ )
		{
			...
			MPI_Send( &BigSignal[dst*PPSize], PPSize+MAXPERIODS, MPI_FLOAT, dst, TAG_SCATTER, MPI_COMM_WORLD );
		}
	}
	else
	{
		// have everyone else receive from the BOSS:
		MPI_Recv( PPSignal, PPSize+MAXPERIODS, MPI_FLOAT, BOSS, TAG_SCATTER, MPI_COMM_WORLD, &status );
	}
